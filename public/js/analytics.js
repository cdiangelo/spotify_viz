...
class AnalyticsManager {
  ...
  async savePlaylistToSpotify() {
    ...
    try {
      ...
      const uris = this.generatedTracks.map(t => t.uri);
      await api.addTracksToPlaylist(playlist.id, uris);

      document.getElementById('save-playlist-btn').textContent = 'Saved!';
      setTimeout(() => {
        document.getElementById('save-playlist-btn').textContent = 'Save to Spotify';
      }, 2000);
    } catch (err) {
